...
class Board:
    ...
    def calc_moves(self, piece, row, col, bool=True):
        ...
        def pawn_moves():
            ...
            directions = [-1, # left en passant
                           1] # right en passant
            for dir in directions:
                if Square.in_range(col+dir) and row == r:
                    p = self.squares[row][col+dir].piece
                    if isinstance(p, Pawn):
                        if p.en_passant:
                            # create initial and final move squares
                            initial = Square(row, col)
                            final = Square(fr, col+dir, p)
                            # create a new move
                            move = Move(initial, final)
        ...
        def knight_moves():
            ...
            for possible_move in possible_moves:
                ...
                if Square.in_range(possible_move_row, possible_move_col):
                    if self.squares[possible_move_row][possible_move_col].isempty_or_isenemy(piece.color):
